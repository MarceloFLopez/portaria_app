...
@Named
public class WRegistroClient implements Serializable {
	...
	public List<WRegistro> listAll() {
		...
		List<WRegistro> t = gson.fromJson(json, new TypeToken<List<WRegistro>>() {
		}.getType());
		return t;
	}

	public WRegistro findId(String id) {
		Gson gson = new Gson();
		Client c = Client.create();
		// GET example
		...
		String json = wr.get(String.class);
		gson = new GsonBuilder().registerTypeAdapter(Date.class, new DateDeserializer()).create();
		WRegistro t = gson.fromJson(json, new TypeToken<Usuario>() {
		}.getType());
		return t;
	}

	public void salvar(WRegistro objeto) {
		Gson gson = new Gson();
		Client c = Client.create();
	...
	public void atualizar(WRegistro objeto) {
		Gson gson = new Gson();
		Client c = Client.create();
